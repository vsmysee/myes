...
import org.elasticsearch.util.gnu.trove.TObjectByteHashMap;
import org.elasticsearch.util.gnu.trove.TObjectByteIterator;
...
public class TObjectByteHashMapDecorator<V> extends AbstractMap<V, Byte>
     ...
     */
    public Set<Entry<V, Byte>> entrySet() {
        return new AbstractSet<Entry<V, Byte>>() {
            public int size() {
                return _map.size();
            }
            ...
            public Iterator<Entry<V, Byte>> iterator() {
                return new Iterator<Entry<V, Byte>>() {
                    private final TObjectByteIterator<V> it = _map.iterator();

                    public Entry<V, Byte> next() {
                        it.advance();
                        final V key = wrapKey(it.key());
                        final Byte v = wrapValue(it.value());
                        return new Entry<V, Byte>() {
                            private Byte val = v;
            ...
            public boolean addAll(Collection<? extends Entry<V, Byte>> c) {
                throw new UnsupportedOperationException();
            }
