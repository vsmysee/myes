...
import org.elasticsearch.util.gnu.trove.TObjectShortHashMap;
import org.elasticsearch.util.gnu.trove.TObjectShortIterator;
...
public class TObjectShortHashMapDecorator<V> extends AbstractMap<V, Short>
     ...
     */
    public Set<Entry<V, Short>> entrySet() {
        return new AbstractSet<Entry<V, Short>>() {
            public int size() {
                return _map.size();
            }
            ...
            public Iterator<Entry<V, Short>> iterator() {
                return new Iterator<Entry<V, Short>>() {
                    private final TObjectShortIterator<V> it = _map.iterator();

                    public Entry<V, Short> next() {
                        it.advance();
                        final V key = wrapKey(it.key());
                        final Short v = wrapValue(it.value());
                        return new Entry<V, Short>() {
                            private Short val = v;
            ...
            public boolean addAll(Collection<? extends Entry<V, Short>> c) {
                throw new UnsupportedOperationException();
            }
