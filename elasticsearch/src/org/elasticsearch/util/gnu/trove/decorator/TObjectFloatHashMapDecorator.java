...
public class TObjectFloatHashMapDecorator<V> extends AbstractMap<V, Float>
     ...
     */
    public Set<Entry<V, Float>> entrySet() {
        return new AbstractSet<Entry<V, Float>>() {
            public int size() {
                return _map.size();
            }
            ...
            public Iterator<Entry<V, Float>> iterator() {
                return new Iterator<Entry<V, Float>>() {
                    private final TObjectFloatIterator<V> it = _map.iterator();

                    public Entry<V, Float> next() {
                        it.advance();
                        final V key = wrapKey(it.key());
                        final Float v = wrapValue(it.value());
                        return new Entry<V, Float>() {
                            private Float val = v;
            ...
            public boolean addAll(Collection<? extends Entry<V, Float>> c) {
                throw new UnsupportedOperationException();
            }
